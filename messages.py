...
def create_category(category_name, is_secret):
    if not users.check_admin():
        return False
    sql = """
    INSERT INTO categories (name, is_secret)
...
def edit_category(category_id, category_name, is_secret, allowed_users):
    ...
    sql = """
    UPDATE categories 
    SET name = :category_name, is_secret = :is_secret, allowed_users = :allowed_users
    WHERE category_id = :category_id
    """
    db.session.execute(text(sql), {"category_name":category_name, 
...
def get_categories():
    ...
    FROM categories c
    LEFT JOIN threads t ON c.category_id = t.category_id
    LEFT JOIN messages m ON t.thread_id = m.thread_id
    LEFT JOIN user_rights ur ON TRUE
    WHERE 
        c.is_secret = FALSE
        OR ur.is_admin = TRUE
...
def check_categories(name):
    sql = """
    SELECT name, category_id, is_secret 
    FROM categories 
    WHERE name = :name
    """
...
def create_thread(content, category_id, thread_name):
    ...
    if user_id == 0:
        return False
    sql = """
    INSERT INTO threads (user_id, name, category_id, content)
    VALUES (:user_id, :name, :category_id, :content)
    RETURNING thread_id
    """
    thread_id = db.session.execute(text(sql), {"user_id":user_id,
                                               "name":thread_name,
                                               "category_id":category_id,
...
def edit_thread(content, thread_name, thread_id):
    ...
    """
    db.session.execute(text(sql), {"thread_name":thread_name,
                                   "content":content,
                                   "thread_id":thread_id,
                                   "user_id":user_id})
    db.session.commit()
...
def send_message(content, category_id, thread_id):
    ...
    if not check_message(content):
        return False
    sql = """
    INSERT INTO messages (content, user_id, thread_id, category_id) 
    VALUES (:content, :user_id, :thread_id, :category_id)
    """
